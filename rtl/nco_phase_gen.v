...
// nco_phase_gen — synthesizable NCO: 256-entry Q1.15 sin/cos ROM + LATENCY-stage pipeline.
// Replaces the behavioral $sin/$cos model from Step 20.
// ROM: index = phase_acc[31:24] (8 MSBs), full 2pi cycle, 256 entries.
// Convention: 0x7FFF=+1.0, 0x0000=0, 0x8001=-1.0 (Q1.15).
// Phase: acc[31:24]=0x40→90°, 0x80→180°, 0xC0→270°.
// Latency: LATENCY pipeline registers, same as behavioral model.
...
module nco_phase_gen #(
    ...
    // --- 256-entry sin/cos ROM (Q1.15, full 2pi cycle) ---
    // index = phase_acc[31:24]
    // Synthesizes as distributed RAM or LUTRAM in Vivado.
    ...
    // --- LUT lookup (combinatorial, pre-accumulation phase) ---
    wire [7:0] lut_idx = phase_acc_r[31:24];
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            // Capture LUT output for PRE-ACCUMULATION phase (lut_idx uses old phase_acc_r).
            // NBA semantics guarantee phase_acc_r is not yet updated at RHS evaluation.
            sin_pipe  [0] <= SIN_ROM[lut_idx];
            cos_pipe  [0] <= COS_ROM[lut_idx];
